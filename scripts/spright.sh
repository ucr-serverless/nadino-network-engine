...
ssh -q $WORKER_1 'bash -s' <<ENDSSH
    ...
    tmux send-keys -t spright:demo.1 "sudo ./run.sh shm_mgr cfg/online-boutique-spright.cfg" Enter
    sleep 10
    tmux send-keys -t spright:demo.2 "sudo ./run.sh gateway cfg/online-boutique-spright.cfg" Enter
    sleep 10
...
# Download results
DOWNLOAD_DIR=palladium_spright_result_$(date +%Y-%m-%d-%H-%M)
mkdir ./$DOWNLOAD_DIR
